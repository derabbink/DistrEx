...
{
    [TestFixture]
    public class NonNestedSequentialTest
    {
        ...
        {
            var chain = Coordinator.Do(_local.Do<int, int>((ct, p, a) => a), _incArgument);
            for (int i = 0; i < chainLength; i++)
            {
                chain = chain.ThenDo(_local.Do(_inc));
            }
            
            Assert.That(chain.ResultValue, Is.EqualTo(chainLength));
        }
    }
}
